...
import { DOCX_HEADING_LEVEL_ATTRIBUTE } from "./docxOutline";
...
import { runCooperativeWork } from "../reader/cooperativeWork";
import {
...
import { TextSearchIndex } from "../reader/textSearchIndex";
import type { ReaderSession } from "../reader/session";
import {
  getReaderRecoveryText,
  type ReaderRecoveryReason,
} from "../reader/recovery";
import { ReaderResourceScope } from "../reader/resourceScope";
import {
  evaluateReaderMemoryPressure,
  readRuntimeHeapBudget,
} from "../reader/memoryPressure";
import { OfficeReaderShell } from "../reader/shell";
import type { ReaderWorkspaceRecoveryReason } from "../reader/workspaceRecovery";
...
const DOM_COMMIT_MAXIMUM_BATCH_SIZE = 16;
const DOCX_DOM_BUDGET = 120_000;
const OUTLINE_CHUNK_SIZE = 50;
const LONG_DOCUMENT_PAGE_THRESHOLD = 12;
const SEARCH_HIGHLIGHT_NAME = "word-reader-search";
const CURRENT_SEARCH_HIGHLIGHT_NAME = "word-reader-search-current";
let nextSearchHighlightId = 0;
...
export class DocxSession extends FileView implements ReaderSession {
  readonly adapter: DocxAdapter = DOCX_ADAPTER;
  ...
  private searchMatchRanges: Range[] = [];
  ...
  private textSearchIndex: TextSearchIndex | null = null;
  private readonly searchHighlightOwnerId =
    `word-reader-view-${nextSearchHighlightId}`;
  private buffer: ArrayBuffer | null = null;
  private extractedPlainText: string | null = null;
  private extractedMarkdown: string | null = null;
  ...
  private documentHeadings: OutlineHeading[] = [];
  ...
  private readonly sessionResources = new ReaderResourceScope((error) => {
    console.error("Office Reader DOCX lifecycle cleanup failed", error);
  });
  private fileRevision: ReaderFileRevision | null = null;
  ...
  constructor(leaf: WorkspaceLeaf, plugin: WordReaderPlugin) {
    ...
    this.fitWidth = plugin.settings.docx.defaultFitWidth;
    this.outlineVisible = plugin.settings.docx.showOutlineByDefault;
  }
  ...
  async onClose(): Promise<void> {
    ...
    this.lastDiagnosticsText = null;
    this.fileRevision = null;
    this.sessionResources.clear();
    await this.sessionResources.drainTasks();
    this.sessionResources.clear();
  ...
  async reload(): Promise<void> {
    ...
    if (this.activeFilePath) {
      ...
      this.restoreReadingState(this.file);
    }
  ...
  handleFileChange(change: ReaderFileChange): void {
    if (
      change.path !== this.activeFilePath &&
      change.path !== this.file?.path
    ) {
      return;
    }
    const reason = change.kind === "renamed" &&
      change.newExtension &&
      !this.adapter.supports({ extension: change.newExtension })
      ? "format-changed"
      : change.kind;
    this.invalidateForRecovery(reason);
  }
  ...
  async openExternal(): Promise<void> {
    if (this.file) {
      await openExternalDocx(this.app, this.file, this.text);
    }
  }
  ...
  async createSummaryNote(): Promise<void> {
    ...
    if (this.isLegacyDocFile(this.file)) {
      new Notice(this.text.notices.legacyDoc);
      return;
    }
    await createNoteFromDocx(this.app, this.file, this.text);
  }
  ...
  async copyText(): Promise<void> {
    ...
    const plainText =
      this.extractedPlainText ??
      this.adapter.extractText(this.documentEl).plainText;
    this.extractedPlainText = plainText;
  ...
  async copyMarkdown(): Promise<void> {
    ...
    const markdown =
      this.extractedMarkdown ??
      this.adapter.extractText(this.documentEl).markdown ??
      "";
    this.extractedMarkdown = markdown;
  ...
  refreshInterfaceLanguage(): void {
    ...
    this.buildLayout();
    if (file) {
      this.sessionResources.runTask(() => this.loadDocx(file, { force: true }));
    }
  }
  ...
  private buildLayout(): void {
    ...
    const layout = this.shell.build({
      rootClasses: "word-reader-root",
      toolbarClasses: "word-reader-toolbar",
      statusClasses: "word-reader-status",
      bodyClasses: "word-reader-body",
    });
    ...
    this.createToolbarButton(
      ...
      text.toolbar.reload,
      () => {
        this.sessionResources.runTask(() => this.reload());
      },
    );
    ...
    this.createToolbarButton(
      ...
      () => {
        ...
        this.restoreScrollPosition(this.renderLifecycle.currentToken).catch(
          () => undefined,
        );
      },
    );

    this.outlineToggleButtonEl = this.createToolbarButton(
      toolbarEl,
      "navigation",
      "list-tree",
      ...
      () => {
        ...
        if (this.outlineVisible && this.renderedDocumentKey) {
          this.sessionResources.runTask(() =>
            this.updateOutline(this.renderLifecycle.currentToken)
          );
        }
      },
    );
    ...
    if (this.capabilities.search) {
      this.searchInputEl = toolbarEl.createEl("input", {
        cls: "office-reader-search word-reader-search",
        attr: {
          type: "search",
      ...
      this.searchPreviousButtonEl = this.createToolbarButton(
        toolbarEl,
        "search",
        "chevron-up",
      ...
      this.searchNextButtonEl = this.createToolbarButton(
        toolbarEl,
        "search",
        "chevron-down",
      ...
      this.searchCountEl = toolbarEl.createSpan({
        cls: "office-reader-search-count word-reader-search-count",
        text: "0",
      });
    } else {
    ...
    this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.copyText());
      },
    );
    ...
    this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.copyMarkdown());
      },
    );
    ...
    this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.createSummaryNote());
      },
    );
    ...
    this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.openExternal());
      },
    );
  ...
  private async loadDocx(
    ...
    this.fileRevision = this.plugin.captureFileRevision(
      file,
      this.adapter.format,
    );
    ...
    const largeFileWarningBytes =
      this.plugin.settings.common.largeFileWarningMb * 1024 * 1024;
    ...
    try {
      ...
      const buffer = await this.adapter.open(this.app, file);
      if (!this.isLoadCurrent(token)) {
        return;
      }

      this.buffer = buffer;
      this.sessionResources.track("caches", "docx-buffer", () => {
  ...
  private async renderCurrentBuffer(
    ...
    try {
      ...
      this.performanceTracker.recordUninterruptibleDocxRender(
        rendererDurationMs,
      );
  ...
  private async commitRenderedDocument(
    ...
    const units: Array<{ parent: Node; node: Node }> = [];
    for (const node of topLevelNodes) {
      if (node.instanceOf(HTMLElement) && node.matches(".docx-wrapper")) {
        const wrapperEl = node.cloneNode(false) as HTMLElement;
        units.push({ parent: targetEl, node: wrapperEl });
        for (const child of Array.from(node.childNodes)) {
          units.push({ parent: wrapperEl, node: child });
        }
      } else {
        units.push({ parent: targetEl, node });
      }
    }
    const pressure = evaluateReaderMemoryPressure({
      ...readRuntimeHeapBudget(),
      domNodes: renderTargetEl.querySelectorAll("*").length,
      domLimit: DOCX_DOM_BUDGET,
    }, 1);
    this.performanceTracker.recordMemoryPressure(pressure.level);
    const maximumBatchSize = Math.max(
      1,
      Math.floor(DOM_COMMIT_MAXIMUM_BATCH_SIZE * pressure.batchScale),
    );
    const diagnostics = await runCooperativeWork(
      ...
      (start, end) => {
        ...
        for (const unit of units.slice(start, end)) {
          if (unit.parent !== parent) {
            flush();
            parent = unit.parent;
            fragment = targetEl.ownerDocument.createDocumentFragment();
          }
          fragment?.appendChild(unit.node);
        }
        flush();
        this.setStatus(
          this.text.status.preparingPreview(
            fileName,
            Math.round((end / Math.max(units.length, 1)) * 100),
          ),
          false,
          true,
        );
      },
      {
        ...
        timeSliceMs: 8,
        isCancelled: () =>
          !this.isLoadCurrent(token) || this.documentEl !== targetEl,
        yieldControl: yieldToBrowser,
      },
    );
    this.performanceTracker.recordWorkSlice(
      diagnostics.maximumWorkSliceMs,
    );
    this.performanceTracker.recordYield(diagnostics.yieldCount);
    this.performanceTracker.recordBackgroundPreemption(
      diagnostics.preemptionCount,
    );
  }
  ...
  private applyDocumentOptions(): void {
    ...
    this.documentEl.toggleClass(
      "can-preview-images",
      this.plugin.settings.docx.enableImagePreview,
    );
    this.updateZoomControl();
  }
  ...
  private async updateOutline(token: number): Promise<void> {
    ...
    this.outlineItems = buildReaderOutline(this.documentHeadings);
  ...
  private scheduleScrollUpdate(): void {
    ...
    this.scrollFrameId = this.sessionResources.requestFrame(scrollWindow, () => {
  ...
  private cancelScrollFrame(): void {
    ...
    const scrollWindow = this.scrollEl?.win ?? window;
    this.sessionResources.cancelFrame(scrollWindow, this.scrollFrameId);
    this.scrollFrameId = null;
  }
  ...
  private handleDocumentClick(event: MouseEvent): void {
    if (!this.plugin.settings.docx.enableImagePreview) {
      return;
    }
  ...
  private showLegacyDocMessage(file: TFile): void {
    ...
    this.createIconButton(actionsEl, "external-link", this.text.toolbar.openExternally, () => {
      this.sessionResources.runTask(() =>
        openExternalDocx(this.app, file, this.text)
      );
    });
  ...
  private showError(error: unknown, file: TFile): void {
    ...
    const formattedDiagnostics = formatWordDiagnostics(diagnostics);
    this.clearRenderedDocument();
    if (!this.documentEl) {
      return;
    }
    const messageEl = this.shell.renderError(this.documentEl, {
      title: errorInfo.title,
      body: errorInfo.body,
      tips: errorInfo.tips,
      detailsLabel: this.text.errors.diagnosticDetails,
      details: formattedDiagnostics,
    });
    ...
    this.createIconButton(
      ...
      () => {
        this.sessionResources.runTask(() =>
          this.writeDiagnostics(formattedDiagnostics)
        );
      },
    );
    this.createIconButton(
    ...
    this.lastDiagnosticsText = formattedDiagnostics;
    this.setStatus(errorInfo.status, true);
  }

  private invalidateForRecovery(reason: ReaderRecoveryReason): void {
    this.activeFilePath = null;
    ...
    const recovery = getReaderRecoveryText(
      this.plugin.settings.common.language,
      reason,
    );
    this.shell.renderRecoveryError(this.documentEl, recovery, () => {
      this.sessionResources.runTask(() => this.reload());
    });
    this.setStatus(recovery.status, true);
  }

  private isLoadCurrent(token: number): boolean {
    return this.renderLifecycle.isCurrent(token) &&
      this.plugin.isFileRevisionCurrent(
        this.fileRevision,
        this.file,
        this.adapter.format,
      );
  ...
  private async writeDiagnostics(diagnostics: string): Promise<void> {
    try {
  ...
  private getSelectedRenderedMarkdown(): string {
    ...
    return docxFragmentToMarkdown(fragment).trim();
  }

  private scheduleSearchHighlights(): void {
    this.clearSearchTimer();
    const taskToken = ++this.searchTaskToken;
    this.searchHighlightTimer = this.sessionResources.setTimer(window, () => {
      this.searchHighlightTimer = null;
      this.sessionResources.runTask(() =>
        this.updateSearchHighlights(taskToken)
      );
    }, SEARCH_DEBOUNCE_MS);
  }
  ...
  private clearSearchTimer(): void {
    ...
    this.sessionResources.clearTimer(window, this.searchHighlightTimer);
    this.searchHighlightTimer = null;
  }

  private async updateSearchHighlights(taskToken: number): Promise<void> {
    if (!this.documentEl || !this.textSearchIndex) {
      return;
    }
    ...
    this.searchMatchRanges = matches.flatMap((match) => {
      if (!match.node.isConnected || !documentEl.contains(match.node)) {
        return [];
      }
      const range = documentEl.ownerDocument.createRange();
      range.setStart(match.node, match.start);
      range.setEnd(match.node, match.end);
      return [range];
    });
    if (this.searchMatchRanges.length > 0) {
  ...
  private updateSearchState(options?: { scrollToCurrent?: boolean }): void {
    const matchCount = this.searchMatchRanges.length;
    this.installCurrentSearchHighlight();
    ...
    if (options?.scrollToCurrent && this.currentSearchIndex >= 0) {
      const range = this.searchMatchRanges[this.currentSearchIndex];
  ...
  private installSearchHighlights(): void {
    setManagedHighlightRanges(
      this.documentEl?.ownerDocument,
      SEARCH_HIGHLIGHT_NAME,
      this.searchHighlightOwnerId,
      this.searchMatchRanges,
    );
  }

  private installCurrentSearchHighlight(): void {
    const range = this.searchMatchRanges[this.currentSearchIndex];
    setManagedHighlightRanges(
      this.documentEl?.ownerDocument,
      CURRENT_SEARCH_HIGHLIGHT_NAME,
      this.searchHighlightOwnerId,
      range ? [range] : [],
    );
  }
  ...
  private restoreReadingState(file: TFile): void {
    ...
    this.fitWidth =
      state?.fitWidth ?? this.plugin.settings.docx.defaultFitWidth;
    this.outlineVisible =
      state?.outlineVisible ??
      this.plugin.settings.docx.showOutlineByDefault;
  ...
  private async restoreScrollPosition(token: number): Promise<void> {
    ...
    await yieldToBrowser();
    if (!this.isLoadCurrent(token) || !this.scrollEl) {
      return;
    }
  ...
  private saveReadingState(): void {
    ...
    const state: ReaderViewState = {
      ...
      collapsedOutlineIds: Array.from(this.collapsedOutlineIds).sort(),
    };
    const serializedState = JSON.stringify(state);
    if (serializedState === this.lastSavedReadingState) {
      return;
    }
    this.lastSavedReadingState = serializedState;
  ...
  private clearSearchHighlightsState(): void {
    ...
    for (const range of this.searchMatchRanges) {
      range.detach();
    }
    this.searchMatchRanges = [];
  ...
  private releaseDocumentState(): void {
    this.cancelSearchWork();
    this.sessionResources.clear("caches");
    this.buffer = null;
    this.extractedPlainText = null;
    this.extractedMarkdown = null;
...
interface RenderedDocumentAnalysis {
  blobUrls: Set<string>;
...
function analyzeRenderedDocument(
  rootEl: HTMLElement,
): RenderedDocumentAnalysis {
  ...
  analyzeRenderedElement(rootEl);
  while (walker.nextNode()) {
    const node = walker.currentNode;
    if (node.nodeType === 1) {
      analyzeRenderedElement(node as HTMLElement);
    } else if (node.nodeType === 3) {
      const textNode = node as Text;
      if (!textNode.parentElement?.closest("style, script")) {
  ...
  return {
    ...
    imageCount,
    pageCount,
    textSearchIndex: TextSearchIndex.fromTextNodes(textNodes),
  };

  function analyzeRenderedElement(element: HTMLElement): void {
    for (const attribute of Array.from(element.attributes)) {
      addBlobUrls(attribute.value, blobUrls);
    }
    if (element.tagName === "STYLE") {
      addBlobUrls(element.textContent ?? "", blobUrls);
    }
    if (element.tagName === "IMG") {
      const imageEl = element as HTMLImageElement;
      imageEl.loading = "lazy";
      imageEl.decoding = "async";
      imageCount += 1;
    }
    if (element.matches(".docx-wrapper > section.docx")) {
      pageCount += 1;
    }
    if (
      element.matches(
...
function addBlobUrls(value: string, urls: Set<string>): void {
  for (const match of value.matchAll(/blob:[^)'"\s]+/g)) {
    urls.add(match[0]);
  }
}
...
interface ManagedHighlightState {
  rangesByName: Map<string, Map<string, readonly Range[]>>;
}

const managedHighlightStates = new WeakMap<Document, ManagedHighlightState>();
...
function getHighlightApi(
  ...
  const view = ownerDocument?.defaultView as (Window & {
...
function setManagedHighlightRanges(
  ownerDocument: Document | undefined,
  ...
  const api = getHighlightApi(ownerDocument);
  if (!api) {
    return;
  }
  let state = managedHighlightStates.get(ownerDocument);
  if (!state) {
    state = { rangesByName: new Map() };
    managedHighlightStates.set(ownerDocument, state);
  }
  let rangesByOwner = state.rangesByName.get(highlightName);
  if (!rangesByOwner) {
    rangesByOwner = new Map();
    state.rangesByName.set(highlightName, rangesByOwner);
  }
  if (ranges.length === 0) {
    rangesByOwner.delete(ownerId);
  } else {
    rangesByOwner.set(ownerId, ranges);
  }

  const combinedRanges = Array.from(rangesByOwner.values()).flat();
  api.registry.delete(highlightName);
  if (combinedRanges.length > 0) {
    api.registry.set(
      highlightName,
      new api.HighlightConstructor(...combinedRanges),
    );
  } else {
    state.rangesByName.delete(highlightName);
  }
}
...
class ImagePreviewModal extends Modal {
  private readonly resources = new ReaderResourceScope((error) => {
    console.error("Office Reader image preview task failed", error);
  });
  ...
  private readonly handleKeyDown = (event: KeyboardEvent): void => {
    ...
    event.preventDefault();
    event.stopPropagation();
    this.resources.runTask(() => this.copyImage());
  };
  ...
  onOpen(): void {
    ...
    this.createToolbarButton(toolbarEl, "download", this.text.imagePreview.saveImageAs, () => {
      this.resources.runTask(() => this.saveImageAs());
    });
