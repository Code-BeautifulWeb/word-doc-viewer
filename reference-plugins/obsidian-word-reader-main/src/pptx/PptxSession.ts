...
import { ReaderWorkCoordinator } from "../reader/cooperativeWork";
import {
...
import { DEFAULT_PPTX_ZIP_LIMITS } from "./zipLimits";
...
export class PptxSession extends FileView implements ReaderSession {
  ...
  private readonly workCoordinator = new ReaderWorkCoordinator();
  private memoryPolicy: ReaderMemoryPolicy = evaluateReaderMemoryPressure({});
  private navigationRenderFrameId: number | null = null;
  private finishNavigationForeground: (() => void) | null = null;
  ...
  private readonly sessionResources = new ReaderResourceScope((error) => {
    console.error("Office Reader PPTX lifecycle task failed", error);
  });
  ...
  private readonly slideResources = new RetainedResourceRegistry((resource) => {
    this.sessionResources.forget("blobUrls", resource);
  ...
  async onOpen(): Promise<void> {
    ...
    this.sessionResources.observe(resizeObserver, this.contentEl);
  ...
  refreshInterfaceLanguage(): void {
    ...
    if (file && this.presentation) {
      ...
      this.sessionResources.runTask(() =>
        this.renderCurrentSlide({ restoreScroll: true })
      );
  ...
  private buildLayout(): void {
    ...
    this.previousButtonEl = this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.previousSlide());
      },
    );
    this.nextButtonEl = this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.nextSlide());
      },
    );
    ...
    if (this.capabilities.paged) {
      ...
      this.pageInputEl.addEventListener("change", () => {
        ...
        if (Number.isInteger(page)) {
          this.sessionResources.runTask(() => this.goToSlide(page - 1));
        } else {
    ...
    this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.copyRenderDiagnostics());
      },
    );
    this.createToolbarButton(
      toolbarEl,
    ...
    this.fullscreenButtonEl = this.createToolbarButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.toggleFullscreen());
      },
    );
    this.createToolbarButton(
      toolbarEl,
      ...
      this.searchInputEl.addEventListener("keydown", (event) => {
        if (event.key === "Enter" && this.searchQuery.trim()) {
          ...
          if (firstResult) {
            ...
            this.sessionResources.runTask(() =>
              this.goToSlide(firstResult.slideIndex)
            );
          }
        }
      });
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    switch (event.key) {
      ...
      case " ":
        event.preventDefault();
        this.sessionResources.runTask(() =>
          event.shiftKey ? this.previousSlide() : this.nextSlide()
        );
        break;
      case "Home":
        event.preventDefault();
        this.sessionResources.runTask(() => this.goToSlide(0));
        break;
      case "End":
        event.preventDefault();
        this.sessionResources.runTask(() =>
          this.goToSlide((this.presentation?.slideCount ?? 1) - 1)
        );
        break;
    }
  }
  ...
  private async loadPresentation(file: TFile): Promise<void> {
    ...
    this.workCoordinator.resetDiagnostics();
    ...
    try {
      ...
      this.sessionResources.track("caches", presentation, () => {
        presentation.clearCaches();
      });
      ...
      this.updateNotes();
      this.updateNavigationControls();
      this.memoryPolicy = this.updateMemoryPolicy(presentation);
      const metadataStartedAt = performance.now();
      this.metadataPromise = presentation.indexSlideMetadata({
        concurrency: Math.max(
          1,
          this.memoryPolicy.backgroundConcurrency,
        ),
        batchSize: 8,
        ...
        isCancelled: () =>
          ...
          this.presentation !== presentation,
        yieldControl: () => this.yieldMetadataWork(token, presentation),
        onWorkSlice: (durationMs) => {
          this.performanceTracker.recordWorkSlice(durationMs);
        },
        onYield: () => {
          this.performanceTracker.recordYield();
        },
        onMetadata: (metadata) => {
      ...
      this.sessionResources.runTask(async () => {
        try {
          ...
          if (
            ...
            console.debug(
              "[Office Reader] PPTX metadata indexing stopped",
              error,
            );
          }
        }
  ...
  private async goToSlide(index: number): Promise<void> {
    ...
    this.updateNavigationControls();
    this.saveReadingState();
    this.sessionResources.runTask(() => this.loadMetadataForSlide(nextIndex));
    await this.renderCurrentSlide({ restoreScroll: true });
    this.performanceTracker.recordTiming(
      "navigationMs",
  ...
  private async renderCurrentSlide(
    ...
    const finishForeground = this.workCoordinator.beginForeground();
    ...
    try {
      ...
      for (const resource of resources) {
        this.sessionResources.track("blobUrls", resource, () => {
          URL.revokeObjectURL(resource);
        });
      }
      ...
      this.performanceTracker.recordWorkSlice(
        rendered.diagnostics.maxWorkSliceMs,
      );
      this.performanceTracker.recordYield(rendered.diagnostics.yieldCount);
  ...
  private scheduleNavigationSearch(): void {
    if (this.searchTimer !== null) {
      this.sessionResources.clearTimer(window, this.searchTimer);
    }
    this.searchTimer = this.sessionResources.setTimer(window, () => {
  ...
  private scheduleNavigationWindowRender(): void {
    ...
    this.finishNavigationForeground = this.workCoordinator.beginForeground();
    this.thumbnailQueue.setPaused(true);
    this.navigationRenderFrameId = this.sessionResources.requestFrame(
      renderWindow,
      () => {
        ...
        try {
          ...
          this.finishNavigationForeground?.();
          this.finishNavigationForeground = null;
          this.thumbnailQueue.setPaused(false);
          this.refreshThumbnailQueue();
        }
      },
  ...
  private createNavigationEntry(index: number): PptxNavigationEntry {
    ...
    entryEl.addEventListener("click", () => {
      this.sessionResources.runTask(() => this.goToSlide(index));
    });
  ...
  private refreshThumbnailQueue(): void {
    ...
    this.memoryPolicy = this.updateMemoryPolicy(presentation);
    if (!this.memoryPolicy.allowPreload) {
      this.thumbnailQueue.clear();
      return;
    }
  ...
  private async renderThumbnail(
    ...
    try {
      ...
      if (
        ...
        this.sessionResources.setTimer(entry.thumbnailEl.win, () => {
          this.refreshThumbnailQueue();
        }, 0);
      }
    }
  ...
  private scheduleMetadataUiRefresh(): void {
    ...
    this.metadataUiFrameId = this.sessionResources.requestFrame(renderWindow, () => {
  ...
  private async getCompleteMetadata(
    ...
    const pending =
      ...
      presentation.indexSlideMetadata({
        ...
        yieldControl: () => this.yieldMetadataWork(
          this.loadLifecycle.currentToken,
          presentation,
        ),
        onWorkSlice: (durationMs) => {
          this.performanceTracker.recordWorkSlice(durationMs);
  ...
  private async yieldMetadataWork(
    token: number,
    presentation: PptxPackage,
  ): Promise<void> {
    const before = this.workCoordinator.snapshot().backgroundPreemptions;
    const continued = await this.workCoordinator.yieldBackground(
      () => yieldToWindow(this.contentEl.win),
      () =>
        !this.loadLifecycle.isCurrent(token) ||
        this.presentation !== presentation,
    );
    const after = this.workCoordinator.snapshot().backgroundPreemptions;
    this.performanceTracker.recordBackgroundPreemption(after - before);
    if (!continued) {
      throw new PptxMetadataCancelledError();
    }
  }

  private updateMemoryPolicy(presentation: PptxPackage): ReaderMemoryPolicy {
    const retainedResources = this.slideResources.size +
      [...this.navigationEntries.values()].reduce(
        (total, entry) => total + entry.resources.size,
        0,
      );
    const policy = evaluateReaderMemoryPressure({
      ...readRuntimeHeapBudget(),
      packageBytes: presentation.zipSummary.totalUncompressedBytes,
      packageLimitBytes: DEFAULT_PPTX_ZIP_LIMITS.maxTotalUncompressedBytes,
      domNodes: this.rootEl?.querySelectorAll("*").length ?? 0,
      domLimit: 3_000,
      cacheEntries: retainedResources,
      cacheLimit: 128,
    });
    this.thumbnailQueue.setConcurrency(
      policy.backgroundConcurrency,
      policy.backgroundConcurrency === 0,
    );
    this.performanceTracker.recordMemoryPressure(policy.level);
    return policy;
  }
  ...
  private showError(error: unknown): void {
    ...
    this.createIconButton(
      ...
      () => {
        this.sessionResources.runTask(() => this.copyDiagnostics());
      },
    );
    this.createIconButton(
      actionsEl,
  ...
  private invalidateForRecovery(reason: ReaderRecoveryReason): void {
    ...
    const messageEl = this.shell.renderRecoveryError(
      this.canvasEl,
      recovery,
      () => {
        this.sessionResources.runTask(() => this.reload());
  ...
  private isLoadCurrent(token: number): boolean {
    return this.loadLifecycle.isCurrent(token) &&
  ...
  private releasePresentation(): void {
    ...
    this.releaseThumbnailResources();
    if (this.presentation) {
      this.sessionResources.forget("caches", this.presentation);
  ...
  private cancelNavigationWork(): void {
    ...
    if (this.navigationRenderFrameId !== null) {
      this.sessionResources.cancelFrame(
        this.contentEl.win,
...
function yieldToWindow(targetWindow: Window): Promise<void> {
  return new Promise((resolve) => {
    ...
    const timeout = targetWindow.setTimeout(finish, 50);
    const frame = targetWindow.requestAnimationFrame(finish);
  });
}
