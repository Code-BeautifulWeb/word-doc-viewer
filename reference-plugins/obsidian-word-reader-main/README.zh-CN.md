...
- 读取期间源文件被替换时会丢弃旧预览、搜索和缓存状态，并显示可恢复的重新
  加载页面，不会提交过期工作。
...
- 在公式栏读取旧式单元格批注的作者和完整文本；即使批注单元格为空白，网格
  仍会显示可发现的批注标记。
- 按工作表锚点预览包内 PNG、JPEG、GIF 和 BMP 绘图。常见柱状图、折线图、
  面积图和饼图只使用工作簿缓存序列；不支持的图表类型会显示明确占位。
- 保留有界的数值条件格式子集：常量 `cellIs` 规则、双色/三色色阶和数据条。
- 创建同名电子表格摘要笔记，记录工作表尺寸/可见性、命名区域、当前选区，
  批注/图片/图表/条件规则数量，并从实际工作簿内容收集最多 200 个显示值预览。
...
- 成功打开 DOCX、PPTX 和 XLSX 后可复制同一结构的 JSON 性能诊断，包含
  包加载/解析/首屏/搜索/导航/滚动耗时、实际 DOM、有界缓存、保留资源、
  最大工作切片、让出/抢占次数和内存压力；不可中断的 DOCX 第三方渲染会单独
  标记。诊断不会包含正文、演讲者备注、单元格值、内部 XML 或 vault 绝对路径。
- 大文件提醒会显示文件大小，并使用设置中的提醒阈值。

## 性能与稳定性

- `npm run lifecycle:check` 会在强制 GC 下分别对 DOCX、PPTX、XLSX 和混合
  顺序执行 100 次打开/重载/导航/切换/关闭循环，并门禁 Blob URL、缓存、任务、
  定时器、观察器、监听器、过期 UI 提交、保留堆趋势、可恢复包错误和未处理
  rejection。完整 JSON、精简趋势点和 Markdown 摘要会写入
  `benchmark-results/`，CI 保存 90 天。
- `npm run performance:check` 以五个独立进程运行快速 CI 样本；
  `npm run large-files:extended` 以三个独立进程覆盖 500 页 DOCX、2,000 页
  PPTX、Excel 行上限稀疏 XLSX 和 50,000 行/400,000 单元格稠密 XLSX。
  扩展门禁每周及发布前运行，不拖慢每次提交，并输出同结构的机器可读趋势产物。
- DOCX DOM 提交、PPTX 元数据/缩略图任务和 XLSX 流式解析共用自适应 8 ms
  工作切片策略；前台导航可抢占后台工作，内存压力升高时降低批次与并发，达到
  临界压力时停止预加载并释放非活动缓存。
...
- 持久化设置带显式 schema 版本，并按 `common`、`docx`、`pptx` 和 `xlsx`
  分区；2.4 之前的扁平设置会自动迁移。损坏或部分写入的数据会按字段回退；
  原始读取失败后不会被自动修复写入覆盖。
- 文件修改、删除、重命名或工作区恢复格式不匹配时，会在过期解析、搜索、
  缩略图、工作表、绘图或 DOM 工作提交前使当前代次失效；合法重复视图彼此隔离，
  不会阻止启动。
- 渲染流程同时使用取消 token 和文件代次，过期结果会被丢弃。
...
- 工作表与共享字符串 XML 会分片解压和校验；加载过程报告进度，稠密工作表
  不保留完整 `sheetData` XML，并可在分片和行之间取消解析。
- 工作表解析、切换、工作簿级搜索、摘要收集和计划中的网格渲染都可取消；
  包缓存、挂载绘图、图表抽样点和图片 Blob URL 均有固定上限，并在重新加载、
  切换工作表/文件和关闭视图时释放。
...
当前工作表和选区，列出工作表尺寸、可见性、命名区域，以及批注/图片/图表/
条件规则数量，并生成最多 200 个显示值预览。同名 Markdown 已存在时同样
只打开、不覆盖。
...
- 旧版 `.xls` 不直接渲染，`.xlsm` 不注册；数据透视表、切片器、完整图表还原、
  现代线程批注、浮动对象编辑、像素级图表还原和复杂条件格式不在 3.2.0
  XLSX 范围内。
...
- **安全 XLSX 丰富内容**：图片仅限包内安全光栅格式，图表预览只读取缓存序列；
  外部绘图、图表和工作簿关系绝不解析。经过评估的
  [XLSM 兼容性矩阵](XLSM_COMPATIBILITY.md) 保持 `.xlsm` 不注册。
